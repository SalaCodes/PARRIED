...
@onready var gamble: Button = $gamble
...
@onready var go_away: Button = $"go away"
...
func _process(delta: float) -> void:
	...
	if !Input.get_connected_joypads().is_empty():
		if not gamble.disabled:
			if Input.is_action_just_pressed("parry"):
				_on_gamble_pressed()
		elif not go_away.disabled:
...
func _on_gamble_pressed() -> void:
	if SoulsHandler.souls >= 50:
		...
		if !gambled_character:
			gamble_text.text = "something went wrong..... giving souls back...."
			souls_animation.play("souls")
			souls_label_gamble.text = "+50 souls"
			SoulsHandler.souls += 50
		else:
...
func dealer_texts():
	...
	match randi_range(1, 15):
		...
		12:
			...
			show_next_letter()
		13:
			dealer_text.text = "Are you feeling the [color=#ff0000]PRESSURE...[/color]..?"
			show_next_letter()
		14:
...
func show_next_letter(speed := 0.06):
	...
	# Use = (or explicit typing) so GDScript doesn't complain
	var full_text = dealer_text.text
	dealer_text.text = ""  # clear display

	# Split into segments: dictionaries with "type" = "tag" or "text", and "content"
	var segments = []
	var buffer = ""
